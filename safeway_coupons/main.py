...
class safeway:
    ...
    def _send_mail(self) -> None:
        ...
        p = subprocess.Popen(
            ["/usr/sbin/sendmail", "-f", email_from, "-t"],
    ...
    def _login(self) -> None:
        ...
        url = (
            ...
            "/oauth2/ausp6soxrIyPrm8rS2p6/v1/authorize?{}".format(
    ...
    def _clip_coupon(self, post_data: Dict[str, Any]) -> bool:
        ...
        if "errorCd" in c:
            raise Exception(
                'Coupon clipping error code: {} ("{}")'.format(
                    c["errorCd"], c["errorMsg"]
                )
            )
    ...
    def _clip_coupons(self) -> None:
        ...
        try:
            ...
            url = (
                ...
                "?offerPgm=PD-CC&storeId={}&transformOfferbyUpc=n".format(
                    self.store_id
                )
            )
            ...
            for offer_type in offers.keys():
                for i, offer in enumerate(offers[offer_type]):
                    self._debug(
                        "Offer data for offer ID {}: {}".format(
                            offer["offerId"], offer
                        ),
                        level=2,
                    )
                    ...
                    if clip_success:
                        ...
                        self._debug(
                            "Error clipping coupon "
                            "{} {}".format(coupon_type, oid)
                        )
                        error_count += 1
                        if error_count >= 5:
                            raise Exception(
                                "Reached error count threshold"
        ...
        if clip_count > 0 or error_count > 0:
            ...
            for section_tuple in clip_counts.items():
                self._mail_append(
                    "    {} => {:d} " "coupons".format(*section_tuple)
                )
            if error_count > 0:
                self.mail_subject += ", {:d} errors".format(error_count)
                self._mail_append(
                    "Coupon clip errors: " "{:d}".format(error_count)
                )
